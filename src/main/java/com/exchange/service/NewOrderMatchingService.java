...
public class NewOrderMatchingService {
    ...
    public void startContinuousMatching(String symbol) {
        synchronized (symbol.intern()) {
            ...
            while (Boolean.TRUE.equals(unmatchedOrders.get(symbol))) {
                ...
                // 調試輸出正在進行匹配的訂單價格與數量
                System.out.println("Matching Orders - Buy ID: " + highestBuyOrder.getId() + " Price: " + highestBuyOrder.getPrice() + " Quantity: " + highestBuyOrder.getUnfilledQuantity());
                System.out.println("Matching Orders - Sell ID: " + lowestSellOrder.getId() + " Price: " + lowestSellOrder.getPrice() + " Quantity: " + lowestSellOrder.getUnfilledQuantity());
    ...
    private void matchOrders(Order buyOrder, Order sellOrder) {
        ...
        // 調試輸出匹配後的未成交數量
        System.out.println("Updated Buy Order - ID: " + buyOrder.getId() + " Unfilled Quantity: " + buyOrder.getUnfilledQuantity());
        System.out.println("Updated Sell Order - ID: " + sellOrder.getId() + " Unfilled Quantity: " + sellOrder.getUnfilledQuantity());

        // 完全成交時從 Redis 中移除，否則更新 Redis 狀態
        if (buyOrder.getUnfilledQuantity().compareTo(BigDecimal.ZERO) == 0) {
            orderbookService.removeOrderFromRedis(buyOrder);
            System.out.println("Buy Order fully matched and removed from Redis - ID: " + buyOrder.getId());
        } else {
            orderbookService.updateOrderInRedis(buyOrder);
            System.out.println("Buy Order partially matched, updated in Redis - ID: " + buyOrder.getId());
        }

        if (sellOrder.getUnfilledQuantity().compareTo(BigDecimal.ZERO) == 0) {
            orderbookService.removeOrderFromRedis(sellOrder);
            System.out.println("Sell Order fully matched and removed from Redis - ID: " + sellOrder.getId());
        } else {
            orderbookService.updateOrderInRedis(sellOrder);
            System.out.println("Sell Order partially matched, updated in Redis - ID: " + sellOrder.getId());
        }

        // 調試輸出匹配完成的結果
        System.out.println("Matched Order - Buy Order ID: " + buyOrder.getId() + ", Sell Order ID: " + sellOrder.getId() + ", Matched Quantity: " + matchQuantity);
