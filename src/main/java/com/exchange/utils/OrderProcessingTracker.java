...
public class OrderProcessingTracker {
    ...
    private static boolean enableBatchSave = false; // 控制是否啟用批量寫入
    ...
    private static void saveToExcel(String filePath) {
        ...
        try {
            ...
            sheet.addCell(new Label(4, 0, "BigDecimal Operation Time (ns)"));
            sheet.addCell(new Label(5, 0, "Object Creation Time (ns)"));
            sheet.addCell(new Label(6, 0, "Total Processing Time (ns)"));
            sheet.addCell(new Label(7, 0, "Untracked Time (ns)"));
            ...
            for (OrderProcessingData data : processingDataList) {
                ...
                sheet.addCell(new Number(4, rowIndex, data.getBigDecimalOperationTime()));
                sheet.addCell(new Number(5, rowIndex, data.getObjectCreationTime()));
                sheet.addCell(new Number(6, rowIndex, data.getTotalProcessingTime()));
                sheet.addCell(new Number(7, rowIndex, data.getUntrackedTime()));
                rowIndex++;
            }
